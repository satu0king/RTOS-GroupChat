 ...
 *
 * Tested on Linux 3.8.11-200.fc18.x86_64 #1 SMP Wed May 1 19:44:27 UTC 2013
...
void runNTP() {
    ...
    char* host_name = "in.pool.ntp.org";  // NTP server host-name.
    ...
    typedef struct {
        ...
        uint8_t
            poll;  // Eight bits. Maximum interval between successive messages.
        ...
        uint32_t
            refTm_f;  // 32 bits. Reference time-stamp fraction of a second.

        uint32_t origTm_s;  // 32 bits. Originate time-stamp seconds.
        uint32_t
            origTm_f;  // 32 bits. Originate time-stamp fraction of a second.
